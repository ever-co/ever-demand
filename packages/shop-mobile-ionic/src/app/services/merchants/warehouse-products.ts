...
export class WarehouseProductsService {
	...
	async getAllWarehouseProducts(geoLocation: any) {
		const res = await this.apollo
			.query({
				query: gql`
					query GetAllWarehouseProducts(
						$geoLocation: GeoLocationFindInput
					) {
						getAllWarehouseProducts(geoLocation: $geoLocation) {
							id
							price
							initialPrice
							count
			...
			.toPromise();
		return res.data['getAllWarehouseProducts'];
	}
}
